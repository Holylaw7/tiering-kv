...
/**
 * 真实执行门禁收敛表 v11（ADR-0227）：每项状态 / 阻塞原因 / 预期消除阶段。
 * 可执行项全绿 + 未执行项精确登记，禁止伪报完成。
 */
public final class GateConvergenceV11 {
    ...
    private static final List<Gate> GATES = List.of(
            new Gate("TD-048", "CI container E2E + fault injection",
                    ...
                    "requires Linux runner", "Phase 46"),
            ...
            new Gate("K8S-001", "kind cluster validation",
                    Status.REGISTERED_RUNNER,
                    "requires Linux runner", "Phase 46"),
            new Gate("REL-001", "release.yml v1.1-v2.8 records",
                    Status.REGISTERED_RELEASE,
                    "requires real tag trigger", "Phase 46"),
            new Gate("BM-001", "cross-machine production benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-machine runner", "Phase 46"),
            new Gate("BM-002", "cross-region RTT/RTO/RPO benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-region runner", "Phase 46"),
            ...
            new Gate("TD-076", "S3/Spot real network credentials",
                    Status.GREEN_JVM,
                    "real handshake JVM-closed; network pending",
                    "Phase 46"),
            new Gate("TD-079", "multi-cloud global one-phase commit",
                    Status.GREEN_JVM, "", "Phase 45"),
            new Gate("TD-080", "multi-table join and window pushdown",
                    Status.GREEN_JVM, "", "Phase 45"));

    private GateConvergenceV11() {
    }
