...
/**
 * 真实执行门禁收敛表 v13（ADR-0241）：每项状态 / 阻塞原因 / 预期消除阶段。
 * 可执行项全绿 + 未执行项精确登记 + 执行记录归档。
 */
public final class GateConvergenceV13 {
    ...
    private static final List<Gate> GATES = List.of(
            new Gate("TD-048", "CI container E2E + fault injection",
                    ...
                    "requires Linux runner", "Phase 48"),
            ...
            new Gate("K8S-001", "kind cluster validation",
                    Status.REGISTERED_RUNNER,
                    "requires Linux runner", "Phase 48"),
            new Gate("REL-001", "release.yml v1.1-v3.0 records",
                    Status.REGISTERED_RELEASE,
                    "requires real tag trigger", "Phase 48"),
            new Gate("BM-001", "cross-machine production benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-machine runner", "Phase 48"),
            new Gate("BM-002", "cross-region RTT/RTO/RPO benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-region runner", "Phase 48"),
            ...
            new Gate("TD-076", "S3/Spot real network credentials",
                    Status.GREEN_JVM,
                    "quota handshake JVM-closed; network pending",
                    "Phase 48"),
            new Gate("TD-079", "global unified one-phase arbitration",
                    Status.GREEN_JVM, "", "Phase 47"),
            new Gate("TD-080", "reinforcement learning pushdown",
                    Status.GREEN_JVM, "", "Phase 47"));

    private GateConvergenceV13() {
    }
