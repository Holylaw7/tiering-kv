...
/**
 * 真实执行门禁收敛表 v10（ADR-0220）：每项状态 / 阻塞原因 / 预期消除阶段。
 * 可执行项全绿 + 未执行项精确登记，禁止伪报完成。
 */
public final class GateConvergenceV10 {
    ...
    private static final List<Gate> GATES = List.of(
            new Gate("TD-048", "CI container E2E + fault injection",
                    ...
                    "requires Linux runner", "Phase 45"),
            ...
            new Gate("K8S-001", "kind cluster validation",
                    Status.REGISTERED_RUNNER,
                    "requires Linux runner", "Phase 45"),
            new Gate("REL-001", "release.yml v1.1-v2.7 records",
                    Status.REGISTERED_RELEASE,
                    "requires real tag trigger", "Phase 45"),
            new Gate("BM-001", "cross-machine production benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-machine runner", "Phase 45"),
            new Gate("BM-002", "cross-region RTT/RTO/RPO benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-region runner", "Phase 45"),
            ...
            new Gate("TD-076", "S3/Spot real credentials",
                    Status.GREEN_JVM,
                    "real HTTP probe JVM-closed; network pending",
                    "Phase 45"),
            new Gate("TD-079", "global one-phase commit scale-out",
                    Status.GREEN_JVM, "", "Phase 44"),
            new Gate("TD-080", "full operator coprocessor pushdown",
                    Status.GREEN_JVM, "", "Phase 44"));

    private GateConvergenceV10() {
    }
