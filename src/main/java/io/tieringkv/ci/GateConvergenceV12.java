...
/**
 * 真实执行门禁收敛表 v12（ADR-0234）：每项状态 / 阻塞原因 / 预期消除阶段。
 * 可执行项全绿 + 未执行项精确登记，禁止伪报完成。
 */
public final class GateConvergenceV12 {
    ...
    private static final List<Gate> GATES = List.of(
            new Gate("TD-048", "CI container E2E + fault injection",
                    ...
                    "requires Linux runner", "Phase 47"),
            ...
            new Gate("K8S-001", "kind cluster validation",
                    Status.REGISTERED_RUNNER,
                    "requires Linux runner", "Phase 47"),
            new Gate("REL-001", "release.yml v1.1-v2.9 records",
                    Status.REGISTERED_RELEASE,
                    "requires real tag trigger", "Phase 47"),
            new Gate("BM-001", "cross-machine production benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-machine runner", "Phase 47"),
            new Gate("BM-002", "cross-region RTT/RTO/RPO benchmark",
                    Status.REGISTERED_RUNNER,
                    "requires cross-region runner", "Phase 47"),
            ...
            new Gate("TD-076", "S3/Spot real network credentials",
                    Status.GREEN_JVM,
                    "real network handshake JVM-closed; "
                            + "network pending", "Phase 47"),
            new Gate("TD-079", "multi-cloud one-phase scale-out",
                    Status.GREEN_JVM, "", "Phase 46"),
            new Gate("TD-080", "window function family and "
                    + "dynamic pushdown",
                    Status.GREEN_JVM, "", "Phase 46"));

    private GateConvergenceV12() {
    }
