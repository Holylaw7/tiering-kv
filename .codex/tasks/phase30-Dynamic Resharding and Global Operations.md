...
```text
✅ 动态重分片（在线拆分/合并，写不中断，路由单调，回滚安全）
✅ 向量分片迁移落地（双写 + 校验 + 原子切换，召回保持）
✅ SQL 写事务（单/跨 Region 2PC，回滚安全，RBAC）
✅ 全球读水位联动（真实复制水位 + 陈旧度 SLA 报告）
✅ SaaS 账单导出（周期结算 + 行项目 + 审计关联）
✅ v1.3.0 发布候选（release.yml 执行）
✅ 分布式查询优化（谓词裁剪 + 结果缓存基准）
✅ 全链路混沌与容量模型（地域故障矩阵 + 容量报告）
✅ 全量回归 >=3700，复制/查询路径零回退
```
