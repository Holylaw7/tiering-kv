...
```text
✅ 分布式 SQL（跨 Region JOIN/聚合，基准达标）
✅ 向量分片（扩展线性 + 重平衡不中断 + 召回保持）
✅ Geo CRDT 规模验证（100 万键收敛 + 时钟校准）
✅ 三地五中心（拓扑 + 故障矩阵演练 + RTO/RPO 报告）
✅ 全球一致性读（strong/bounded 模式 + 陈旧度报告）
✅ SaaS 计费/市场（计量正确性 + 配额降级）
✅ v1.2.0 发布候选（release.yml 执行）
✅ 分布式可观测性与告警（指标/规则/追踪）
✅ 全量回归 >=3450，复制路径零回退
```
