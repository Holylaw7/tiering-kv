...
```text
✅ 双向复制（版本向量 + CRDT 收敛，无环回风暴）——已完成（ADR-0114）
✅ 两地三中心容灾（计划/故障切换 + DR 演练报告）——已完成（ADR-0115）
✅ SQL JOIN/聚合/优化器（基准达标）——已完成（ADR-0116）
✅ HNSW + 混合检索（召回率/延迟对比报告）——已完成（ADR-0117）
✅ SaaS 多租户控制平面（租户隔离 + 审计）——已完成（ADR-0118）
✅ RPC 帧级令牌（v1 兼容）——已完成（ADR-0119）
✅ v1.1.0 发布候选（release.yml 执行）——流水线扩展完成，执行待 Runner
✅ 跨地域生产基准（RTT/RTO/RPO 如实记录）——进程内口径完成，跨机待执行
✅ 全量回归 >=3200，单向路径零回退——3216/3216 PASS（新增 251）
```
