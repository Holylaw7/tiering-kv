...
```text
✅ SQL 写 2PC 真实接线（与原生 2PC 语义等价）
✅ 控制台 REST 服务（HTTP + RBAC + 自服务）
✅ 自动重分片并发迁移（限速 + 校验 + 回滚 + 熔断）
✅ 全球多活网关冲突审计（冲突完整审计 + 读水位）
✅ 全局多活自动选主（故障矩阵切换 + 防脑裂）
✅ 跨云数据主权（驻留策略 + 违规拒绝）
✅ 跨地域真实基准（RTT/冲突率/收敛/RTO 如实记录）
✅ v1.5.0 发布候选（release.yml 执行）
✅ 全量回归 >=4200，复制/查询/重分片/多活路径零回退
```
