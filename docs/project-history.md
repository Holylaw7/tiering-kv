...
> **归档说明（2026-08-16）**：本文档为 README 历史版本全文
> （截至 Phase 54 / v3.6.0 RC，含 Phase 0–54 逐阶段演进记录与
> 早期能力/基准/发布流水线说明），由 Git 历史原样恢复归档。
> 项目最终状态与使用说明见 [README.md](../README.md)，
> 完整路线图见 [ROADMAP.md](../ROADMAP.md)，
> 最终收尾报告见 [docs/review/final-project-closure.md](review/final-project-closure.md)。
...
以及真实 Runner 闭环 v11 与多云全球一致性：真实执行门禁收敛表 v11
（ADR-0227，可执行项 JVM 全绿 + 未执行项精确登记）、跨云全局一阶段
（ADR-0228，多数云仲裁 + 回退 2PC + resolved-ts 联动）、多表 JOIN /
窗口函数下推（ADR-0229，N 表等值连接 + ROW_NUMBER/RANK + 下推成本
模型）、TSO 全球统一时钟（ADR-0230，GPS/原子钟/NTP 混合授时 + 校准 +
单调 + 恢复不回退）、自治 PD 无人值守（ADR-0231，EWMA 自校准 +
合规证明自动化 + 熔断）、TiKV 跨机对比基线 + 真实凭据 v3（ADR-0232，
跨机口径 + 认证握手探测，TD-076 剩余项）、v2.8.0 冻结与发布流水线
（ADR-0233）；全量测试 ≥9942 全绿（Phase 45，v2.8.0 发布候选）。
以及真实 Runner 门禁闭环与全球一致性最终化：真实执行门禁收敛表 v12
（ADR-0234，可执行项 JVM 全绿 + 未执行项精确登记）、跨云一阶段规模化
（ADR-0235，云 × 区混合拓扑 + 分层仲裁 + 回退 2PC）、窗口函数全族 /
动态下推（ADR-0236，LAG/LEAD/SUM/COUNT/AVG OVER + 运行时成本感知）、
TSO 跨云授时仲裁 + 防时钟回拨（ADR-0237，多数云共识 + 回拨窗口冻结 +
告警）、自治无人值守全自动合规证明（ADR-0238，审计链 + SHA-256 签名 +
外部审计接口）、TiKV 跨机基准定期回归 + 真实凭据 v4（ADR-0239，
趋势/快照 + 可达性/认证/权限，TD-076 剩余项）、v2.9.0 冻结与发布流水线
（ADR-0240）；全量测试 ≥10491 全绿（Phase 46，v2.9.0 发布候选）。
以及真实 Runner 闭环归档与全球一致性 GA：真实执行门禁收敛表 v13 +
执行归档（ADR-0241，RunnerExecutionArchive + 可执行项全绿 + 未执行项
精确登记）、跨云一阶段全球统一仲裁（ADR-0242，任意拓扑自动发现 +
拓扑版本幂等）、RL 动态下推（ADR-0243，Q 学习在线决策，语义层一致）、
TSO 量子/卫星授时原型（ADR-0244，QUANTUM/SATELLITE/HYBRID + 传播
延迟校正 + 防回拨）、监管级合规证书（ADR-0245，时间戳证书 + 密钥轮换
+ 外部验证）、TiKV 跨机回归告警 + 真实凭据 v5（ADR-0246，快照/趋势/
阈值 + 可达性/认证/权限/配额，TD-076 剩余项）、v3.0.0 冻结与发布
流水线（ADR-0247）；全量测试 ≥11053 全绿
（Phase 47，v3.0.0 GA 候选）。
以及真实 Runner 门禁全量闭环与多组织联邦一致性：真实执行门禁收敛表
v14 + 发布记录归档（ADR-0248，ReleaseRecordArchive + 可执行项全绿 +
未执行项精确登记）、多组织联邦仲裁（ADR-0249，cloud → organization
映射 + 组织级仲裁 + 回退 2PC）、RL 多智能体下推（ADR-0250，加权 Q
聚合 + 反馈闭环，语义层一致）、TSO 量子/卫星硬件适配（ADR-0251，
HardwareClock 接口 + 模拟实现 + 故障降级）、监管法规自动映射 +
证据链（ADR-0252）、TiKV 跨机回归闭环 + 真实凭据 v6（ADR-0253，
自动重跑/趋势/告警 + 可达性/认证/权限/配额/延迟，TD-076 剩余项）、
v3.1.0 冻结与发布流水线（ADR-0254）；全量测试 ≥11625 全绿
（Phase 48，v3.1.0 发布候选）。
以及真实 Runner 闭环归档与跨监管域联邦一致性：门禁收敛 v15（唯一
终态 CLOSED/ENV_BLOCKED/REGISTERED_RELEASE + 趋势报表，ADR-0255）+
跨监管域联邦仲裁（域级仲裁 + 回退 2PC，ADR-0256）+ RL 多智能体联邦
学习（FedAvg + 噪声注入 + 梯度裁剪，ADR-0257）+ 商用授时设备接入
