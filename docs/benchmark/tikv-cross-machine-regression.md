# TiKV 跨机基准定期回归（ADR-0239）

## 口径声明

本基线为**本地进程内**口径（JVM 单机、page cache 热），与 TiKV 公开
数据对比仅作数量级参考。**跨机待执行**：BM-001/002 部署脚本
（Gateway×3 / Metadata×3 / Storage×6）已就绪，由 Phase 47 Runner
执行；定期回归（快照 + 趋势 + 告警）以登记为准，禁止伪报。
...
| D | 规模化一阶段 + 窗口全族 + 仲裁（进程内模拟） | 提交 P99 < 1ms；窗口 > 50K rows/s |
