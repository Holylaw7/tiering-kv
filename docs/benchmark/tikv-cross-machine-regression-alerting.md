# TiKV 跨机基准定期回归 + 趋势告警（ADR-0246）

## 口径声明

本基线为**本地进程内**口径（JVM 单机、page cache 热），与 TiKV 公开
数据对比仅作数量级参考。**跨机待执行**：BM-001/002 部署脚本
（Gateway×3 / Metadata×3 / Storage×6）已就绪，由 Phase 48 Runner
执行；定期回归（快照 + 趋势 + 告警阈值）以登记为准，禁止伪报。
...
| D | 统一仲裁 + RL 下推 + 量子授时（进程内模拟） | 提交 P99 < 1ms；RL > 10K/s |
...
- 多机部署脚本 + 对比表快照（吞吐 / 延迟 / 内存）+ 趋势记录；
- 偏离基线超过阈值 → 告警事件；
- 每阶段全量回归基准：`PHASE47-BASELINE` 输出。
