...
| M2 | v4.0-M2 | 多模型编码 | 类型化值（SQL/JSON/时序/向量）additive 编码、RESP3 类型接线、TTL/过期/迁移对多模型值兼容 | ADR-0320 | ✅ 阶段交付中（2026-08-14）：ValueType 6/7/8 + MultiModelCodec + RESP3 映射 + 命令族 + WAL/SSTable/复制闭环 + 基准报告 |
