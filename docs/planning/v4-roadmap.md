...
| M4 | v4.0-GA | 生产收口 | Operator 完整化、多云部署深化、Jepsen 外部化分区注入、真实 Runner 性能基线（冷/热口径） | ADR-0322 | ✅ 完成（2026-08-14，v4.0.0-rc1）：CapacityModel + Operator 状态机 + K8s controller 接线（fabric8）+ 多集群拓扑/计划器 + Jepsen 外部化 + 真实客户端 RESP 链路 + 故障切换演练 + 冷热基线（6.3x）+ 门禁 7/7 |
